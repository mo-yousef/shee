				...
				<div class="shecy-space-y-6">
					...
					<div class="shecy-grid shecy-grid-cols-1 md:shecy-grid-cols-2 shecy-gap-6">
						<div>
							...
							<input type="text" name="business_address" id="business_address" value="<?php echo esc_attr( get_post_meta( $business_id, 'business_address', true ) ); ?>" class="shecy-mt-1 shecy-block shecy-w-full shecy-border-gray-300 shecy-rounded-md shecy-shadow-sm">
						</div>
						<div>
							<label for="business_city">City</label>
							<select name="business_city" id="business_city" class="shecy-mt-1 shecy-block shecy-w-full shecy-border-gray-300 shecy-rounded-md shecy-shadow-sm">
					...
					<div>
						...
						<div class="shecy-mt-1 shecy-flex shecy-flex-wrap shecy-gap-4">
							<?php
							$gallery_ids = get_post_meta( $business_id, 'business_gallery_ids', true );
							if ( ! empty( $gallery_ids ) ) {
								foreach ( $gallery_ids as $attachment_id ) {
									echo '<div class="shecy-w-24 shecy-h-24">' . wp_get_attachment_image( $attachment_id, 'thumbnail' ) . '</div>';
								}
							} else {
						...
						<label for="business_images" class="shecy-mt-4">Upload New Images</label>
						<input type="file" name="business_images[]" id="business_images" accept="image/*" multiple class="shecy-mt-1 shecy-block shecy-w-full">
					</div>
				</div>
