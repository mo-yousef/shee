				...
				<div class="space-y-6">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div>
							...
							<input type="text" name="business_address" id="business_address" value="<?php echo esc_attr( get_post_meta( $business_id, 'business_address', true ) ); ?>" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
						</div>
						<div>
							<label for="business_city">City</label>
							<select name="business_city" id="business_city" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
					...
					<div>
						...
						<div class="mt-1 flex flex-wrap gap-4">
							...
							if ( ! empty( $gallery_ids ) ) {
								foreach ( $gallery_ids as $attachment_id ) {
									echo '<div class="w-24 h-24">' . wp_get_attachment_image( $attachment_id, 'thumbnail' ) . '</div>';
								}
							} else {
						...
						<label for="business_images" class="mt-4">Upload New Images</label>
						<input type="file" name="business_images[]" id="business_images" accept="image/*" multiple class="mt-1 block w-full">
					</div>
				</div>
